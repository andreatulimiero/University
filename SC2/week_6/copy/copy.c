    ...
    while (1) {
        ...
        while (bytes_left > 0) {
             ...
             * the two cases described above. */
             ...
             int ret = read(src_fd, buf + read_bytes, DEFAULT_BLOCK_SIZE);
            ...
            ERROR_HELPER(ret, "Cannot read from src file");
        ...
        while (bytes_left > 0) {
            /** INSERT CODE TO WRITE DATA HERE
             *
             ...
             * the two cases described above. */
             ...
             int ret = write(dest_fd, buf + written_bytes, read_bytes);
             ...
             ERROR_HELPER(ret, "Cannot write on dest file");
