...
int main(int argc, char* argv[]) {
    ...
    while ( (msg_len = recv(socket_desc, buf, sizeof(buf) - 1, 0)) < 0 ) {
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (ret == 0) {
                ERROR_HELPER(-1, "[WARNING] Endpoint closed the connection unexpectedly. Exiting...\n");
            }

            // we use post-increment on msg_len so we first read the
            // byte that has just been written and then we increment
            if (buf[msg_len++] == '\n') break;
        }
        
        printf("Server response: %s", buf); // no need to insert '\0'
    }
