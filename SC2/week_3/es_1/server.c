...
void cleanup() {
    ...
    int ret = sem_unlink(SEMAPHORE_NAME);
    ERROR_HELPER(ret, "Error unlinking sempahore");
...
int main(int argc, char* argv[]) {
     ...
     *
     * O_CREAT tells the system to create the semaphore named SEMAPHORE_NAME
    ...
    if (named_semaphore == SEM_FAILED && errno == EEXIST) {
        ...
        // now we can retry to create the semaphore
        named_semaphore = sem_open(SEMAPHORE_NAME, O_CREAT, 0600, NUM_RESOURCES);
    }
