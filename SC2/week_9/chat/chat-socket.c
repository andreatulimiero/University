...
void* receiveMessage(void* arg) {
    ...
    while (!shouldStop) {
         ...
         *   case, set shouldStop = 1 and exit from the current thread
        ...
        while (1) {
            ...
            if (ret == -1 && errno == EINTR) continue;
            ERROR_HELPER(ret, "Error reading from socket");
            if (buf[bytes_read++] == '\n') break;
        }
...
void* sendMessage(void* arg) {
    ...
    while (!shouldStop) {
         ...
         *
         * Suggestions:
         * - make sure that all bytes have been written!
         * - repeat the operation if interrupted before writing any data
         * - check for errors
         **/
         ...
         while(bytes_written < msg_len) {
            ...
            if (ret == -1 && errno == EINTR) continue;
            ERROR_HELPER(ret, "Error sending to client");
            if (ret > 0 && ret < msg_len) continue;
            bytes_written += ret;
         }
...
void chatSession(int socket_desc) {
    ...
    ret = pthread_create(&chat_threads[0], NULL, receiveMessage, &socket_desc);
    PTHREAD_ERROR_HELPER(ret, "Cannot create thread for receiving messages");

    ret = pthread_create(&chat_threads[1], NULL, sendMessage, &socket_desc);
