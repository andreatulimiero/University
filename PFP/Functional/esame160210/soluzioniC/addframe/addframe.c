...
void addframe(unsigned char* in, int w, int h, 
    ...
    *out = malloc((*ow)*(*oh)*sizeof(unsigned char));
    ...
    err |= clSetKernelArg(kernel, 4, sizeof(unsigned char), &frame_grey);
    err |= clSetKernelArg(kernel, 5, sizeof(int), &frame_size);
    clut_check_err(err, "failed to set kernel arguments");

    // execute the kernel over the range of our 2D output data set
